...
<body class="w-full h-full flex justify-center items-center flex-col">
    <main class="text-black flex justify-start items-center flex-col lg:justify-center lg:w-amna-app md:w-amna-app-content-md lg:h-screen">
        <div class="w-full py-4 mt-4 rounded-md transition-all duration-200 lg:mt-0 md:w-[25rem] md:shadow-2xl">
            <div class="h-full w-full flex flex-col">
                ...
                <div class="w-full px-7">
                    <form class="flex justify-start items-start flex-col" action="{{ route('events.destroy', $event) }}" method="post">
                        @csrf
                        @method('delete')
                        <div>
                            <p>Tem certeza que deseja deletar o evento <span class="font-bold">{{ $event->title }}?</span></p>
                            <p class="py-2">Essa ação não poderá ser desfeita.</p>
                        </div>
